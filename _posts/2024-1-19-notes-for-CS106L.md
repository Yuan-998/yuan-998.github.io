...
- Scope Resolution and Ternay (**::** and **?**)
- Member Access and Pointer-to-member access (**.** and **.***)
- Object size, type, and casting (**sizeof()**, **typeid()**, and **cast()**)

Overload operators in two ways:
- Member functions: access member variables using `this->`
- Non-member functions: the LHS to be a non-class type
...
```c
void* new operator new(size_t size);
```
...
class Widget {
  public:
    ...
    ~Widget();                              // destructor
    Widget(Widget&& rhs);                   // move constuctor
...
- By default, the copy constructor will create copies of each member variable, which is bad for pointer.
- If the variable is a pointer, a member-wise copy will point to the same allocated data, not a fresh copy!

#### We can delte special member functions!
...
class PasswordManager {
  public:
    ...
    // other methods ...
    PasswordManager(const PasswordManger& rhs) = delete;
...
#### Rules for overriding special member functions
- If the default SMFs work, **don't definee your own!**. We should only define new ones when the default ones generated by the compiler won't work. This usually happens when we work with dynamically allocated memory, like pointers to things on the heap!
- If you have to define a **destructor**, **copy constructor**, or **copy assignment operator**, **you should define all three**!


```c++
using std::vecor;
vector<int> func(vector<int> vec0) {    // copy constructor
  vector<int> vec1;                     // default constructor
  vector<int> vec2(3);                  // Not a SMF, a constructor with parameters
  vector<int> vec3{3};                  // Not a SMF, uses an initializer list
...
class StringTable {
  public:
    StringTable() {}
    StringTable(const StringTable& st) {} //copy every value in the values map one by one! Very slowly!
    // ...
